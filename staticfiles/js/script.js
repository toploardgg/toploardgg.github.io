  ...
  if (!document.querySelector("#toast-style")) {
    ...
    s.textContent = `
      ...
      .toast{
        ...
        padding:13px 10px 13px 20px;border-radius:18px;
        font-size:13px;font-weight:600;color:rgba(233,238,248,0.92);
        background:rgba(255,255,255,0.06);backdrop-filter:blur(24px);
        -webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.13);
        box-shadow:0 8px 32px rgba(0,0,0,0.3);
        animation:toastIn .38s cubic-bezier(0.34,1.56,0.64,1) both;
        white-space:nowrap;min-width:220px;max-width:90vw;
      }
      ...
      @media(max-width:600px){
        .toast{bottom:20px;padding:14px 10px 14px 18px;border-radius:16px;font-size:14px;}
...
(function () {
  ...
  function showPhotoPreview(dataUrl, fileName) {
    ...
    wrap.className = "c-photo-preview-wrap";
    ...
    removeBtn.className = "c-photo-remove-btn";
    ...
    photoDrop.appendChild(wrap);
    _photoPreviewEl = wrap;
  }
  ...
  function renderCard(comment, prepend) {
    ...
    const deleteHtml = token
      ? `<button class="c-delete-btn" data-id="${comment.id}" title="Delete your comment" style="
          ...
          display:flex;align-items:center;justify-content:center;
          border-radius:8px;border:1px solid rgba(220,60,60,0.35);
          background:rgba(200,50,50,0.18);color:rgba(255,110,110,0.9);
          font-size:13px;cursor:pointer;
          transition:background .2s,border-color .2s,transform .2s;padding:0;
        "
  ...
  (function () {
    ...
    const countEl = document.getElementById("blogLikeCount");
    const icon = btn && btn.querySelector(".blog-like-icon");
    if (!btn) return;
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        const res = await fetch("/api/blog/likes", { method, headers: { "X-CSRFToken": getCsrfToken() } });
        const data = await res.json();
    ...
    if (document.querySelector(".blog-img")) {
      document.querySelector(".blog-img").addEventListener("click", () => {
        openLightbox(document.querySelector(".blog-img").src);
      });
    }
  ...
  async function init() {
    ...
    try {
      ...
      else comments.forEach((c) => renderCard(c, false));
    } catch {
      listEl.innerHTML =
...
function selectLang(lang) {
  ...
  if (currentLang) currentLang.innerText = lang;
  const dropdown = document.getElementById("langDropdown");
...
window.addEventListener("click", (event) => {
  const dropdown = document.getElementById("langDropdown");
  if (dropdown && !event.target.closest(".lang-dropdown")) {
